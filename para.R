...
find_outliers <- function(x, threshold = 1.5) {
  q1 <- quantile(x, 0.25)
  q3 <- quantile(x, 0.75)
  iqr <- q3 - q1
...
outlier_indices <- apply(scaled_dataset, 2, find_outliers)

# Remove outliers
cleaned_dataset <- scaled_dataset[!apply(outlier_indices, 1, any), ]

# Scale the data
scaled_dataset <- scale(cleaned_dataset)

# Perform PCA
pca_result <- prcomp(scaled_dataset)
...
transformed_dataset <- as.data.frame(selected_components)
...
nb_clusters_pca <- NbClust(transformed_dataset, min.nc = 2, max.nc = 10, method = "kmeans")
...
wcss_pca <- numeric(10)
for (i in 1:10) {
  kmeans_model_pca <- kmeans(transformed_dataset, centers = i)
...
gap_stat_pca <- clusGap(transformed_dataset, FUN = kmeans, nstart = 25, K.max = 10, B = 50)
...
silhouette_score <- function(k){
  ...
  ss <- silhouette(km$cluster, dist(transformed_dataset))
  mean(ss[, 3])
}
k <- 2:10
...
kmeans_model_pca <- kmeans(transformed_dataset, centers = optimal_k)
...
# Plot clusters
plot(transformed_dataset[,1], transformed_dataset[,2], col = kmeans_model_pca$cluster,
